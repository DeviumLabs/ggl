...
export default function Footer() {
  const pushDL = useCallback((event, params) => dlPush(event, params), []);

  const sendFooterNavEvent = useCallback((label, url) => {
    pushDL("navigation_click", { location: "footer", link_text: label, link_url: url });
  }, [pushDL]);

  const sendFooterCallEvent = useCallback((number) => {
    pushDL("click_to_call", { location: "footer", phone_number: number });
  }, [pushDL]);

  const sendSocialClickEvent = useCallback((platform, url) => {
    pushDL("social_click", { location: "footer", platform, link_url: url });
  }, [pushDL]);
  ...
  return (
    <Fragment>
      <footer className="tw-bg-blue tw-pt-[30px] tw-pb-[10px] tw-px-[20px] tw-w-full">
        ...
        <div className="tw-text-center tw-border-white tw-border-t-[1px] tw-mt-[40px] tw-pt-[5px]">
          <a href="https://www.devlara.com.br/" target="_blank" rel="noopener noreferrer" className="tw-text-white hover:tw-underline">
